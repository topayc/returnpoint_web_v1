...
function gpointDetailView(f, searchtime){
	...
	$.ajax({
		...
		success: function(data) {
			returnHtml += "<i class='fa fa-sort-down'></i>"	  	
			returnHtml += "<div class='tabtitle'> 회원(노드명) 적립 내역 상세</div>"
...
<body class="p_point">	
	...
	<!-- content begin -->	
	<ul class="submenu">
		<li class="active"><a href="./newpoint.do"><spring:message code="label.pointinquiry"/></a></li>
		<!-- 포인트전환페이지 <li><a href="./point_transfer.do"><spring:message code="label.switchPoints"/></a></li> -->
		<li><a href="./point_gift.do"><spring:message code="label.pointGift"/></a></li>
	</ul>
	<section>
		<%-- <hr><c:set var="serverDateDiff" value="${model.serverDateDiff}" />		
		<div class="pointdate"><a href="/mypage/newpoint.do?move=${model.PREWMONTH}"> <i class="fas fa-chevron-circle-left"></i> </a>
		<span>${model.NOWMONTH}</span>
		<input type="hidden" id="NOWMONTH" name="NOWMONTH" value="${model.NOWMONTH}"/>
		<c:choose>
		<c:when test="${model.NOWMONTH eq serverDateDiff}">
		<a href="#"  class="disable"> <i class="fas fa-chevron-circle-right"></i></a>
		</c:when>
		<c:otherwise>
		<a href="/mypage/newpoint.do?move=${model.NEXTMONTH}"> <i class="fas fa-chevron-circle-right"></i></a>
		</c:otherwise>
		</c:choose>
		</div> --%>
		<ul>
			<a data-toggle="tab" href="#GpointM"><li class="gpoint active"><p class="rpt"><spring:message code="label.gPointEarningHistory"/></p><p><fmt:formatNumber value="${model.myGreenPointSumInfo.greenPointAmountSum}" pattern="###,###,###,###"/></p></li></a>
			<a data-toggle="tab" href="#RpointM"><li class="rpoint"><p class="gpt"><spring:message code="label.rPointconversionHistory"/></p><p><fmt:formatNumber value="${model.myRedPointSumInfo.redPointAmountSum}" pattern="###,###,###,###"/></p></li></a>
		</ul>
		...
		<!-- 그린포인트 노드별 포인트 합 -->
		    <div id="GpointM" class="tab-pane fade in active">	
				...
				<div class="tabtitle"><spring:message code="label.gPointEarningHistory"/></div>	
		    ...
		    <!-- 레드포인트 내역  -->
		    <div id="RpointM" class="tab-pane ">	
		    	<i class="fa fa-sort-down"></i> 
				<div class="tabtitle"><spring:message code="label.rPointconversionHistory"/></div>						
				<div class="listbox">
					<div class="listmember"><span class="node nd1">회원</span></div>
