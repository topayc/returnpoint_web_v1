...
$(document).ready(function(){
 ...
 /* 	makeContactList(
		JSON.stringify([
			{name : "안영철1", phoneNumber : "010-8822-7467"}
		])
	); */
});
...
function sendRecommendSMSCheck(memberName, memberEmail){
	...
	$("input[type='checkbox']:checked").each(function(index,item){
		smsData.push({
			message :  window.location.protocol + "//" + window.location.host + "/m/member/join.do?recommender=" +btoa(encodeURIComponent(parameters["recommender"])),
			phoneNumber : $(this).attr('phoneNumber'),
			url : window.location.protocol + "//" + window.location.host + "/m/member/join.do?recommender=" +btoa(encodeURIComponent(parameters["recommender"]))
		})
	});
